...
def compute_advantage(gamma, lmbda, td_delta):
    ...
    for delta in td_delta[::-1]:
        advantage = gamma * advantage + delta
        advantage_list.append(advantage)
    advantage_list.reverse()
    return tf.convert_to_tensor(advantage_list, dtype=tf.float32)

class REINFORCE:
    def __init__(self, actor_lr, critic_lr,lmbda, epochs, eps, gamma,epsilon_start=0.95, epsilon_end=0.05, epsilon_decay_steps=5000):
    ...
    def save_model(self):
        self.actor3.save(f"actor_model_all_20-250___.h5")
        return
    ...
    def update(self, transition_dict):
        ...
        with open('rl_info2.txt', mode='a+', encoding='utf-8') as fout:
            fout.write(f"price: {price}\n")
            fout.write(f"rewards: {rewards}\n")
            fout.write(f"advantages: {advantage}\n")
        ...
        epsilon = 1e-30
        old_safe_action_probs = tf.clip_by_value(tf.reshape(tf.gather(tf.reshape(action_probs, [-1]), indices), (-1, 1)),
                                             epsilon, 1.0 - epsilon)
        ...
        for _ in range(self.epochs):
            with tf.GradientTape() as actor_tape, tf.GradientTape() as critic_tape:
                ...
                log_probs = tf.math.log(safe_action_probs)
                #actor_loss = -tf.reduce_mean(log_probs * rewards)
                ...
                smoothness_coeff = 10   
                if entropy < 1.5:
                    entropy_coefficient = 50  
                if smoothness_loss(action_probs) > 0.03:
                    smoothness_coeff = 50
                if self.time>=2500:
                    entropy_coefficient=5.0
                    smoothness_coeff=5.0
                    # if entropy < 1.5:
                    #     entropy_coefficient = 100
                    # if smoothness_loss(action_probs) > 0.03:
                    #     smoothness_coeff = 100

                    # entropy_coefficient = 2
                    # smoothness_coeff = 2

                if self.time>=3500:
                    progress = (self.time - 3500) / (5000 - 3500)
                    entropy_coefficient =5.0- 5.0 * progress
                    smoothness_coeff = 5.0 - 5.0 * progress
                print(self.time)
...
log_dir = "logs_all_change/live_multiple_lists"
...
agent = REINFORCE(actor_lr, critic_lr, lmbda,epochs, eps, gamma)


# checkpoint_dir = os.path.join(log_dir, "checkpoints")
# os.makedirs(checkpoint_dir, exist_ok=True)
# agent.setup_checkpoint_manager(checkpoint_dir, max_to_keep=3)
#

start_episode = 0
# train_state_path = os.path.join(checkpoint_dir, "train_state.npy")
#

# if os.path.exists(train_state_path):
#     train_state = np.load(train_state_path, allow_pickle=True).item()
#     start_episode = train_state['episode'] + 1
#     agent.epsilon = train_state['epsilon']
#     agent.time = train_state['time']
#     replay_buffer = train_state['replay_buffer']
#     print(f"Resuming training from episode {start_episode}")
#

#     checkpoint_path = train_state.get('checkpoint_path')
#     if checkpoint_path:
...
for episode in range(start_episode,num_episodes):
    ...
    #cost = sorted([random.randint(25, 150) for _ in range(3)], reverse=True)
    ...
    feature_num=21 
    buffer_ns_input = process_tensors(buffer_ns,feature_num)
    buffer_s_input = process_tensors(buffer_s,feature_num)
    ...
    # td_target = buffer_r + gamma * agent.critic(buffer_ns_input) * (1 - buffer_d)
    # td_delta = td_target - agent.critic(buffer_s_input)
    advantage = compute_advantage(gamma, lmbda, buffer_r)
